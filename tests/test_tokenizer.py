...
def test_tokenize_url_1(tokenizer):
    s1 = "Here is my website: http://t.co/EZWeDhjl, check it out! "
    assert (
        " ".join(tokenizer.tokenize(s1))
        == "Here is my website : <URL> , check it out !"
    )
...
def test_tokenize_url_2(tokenizer):
    ...
    assert (
        " ".join(tokenizer.tokenize(s1))
        == "Here is my http site ! ! ! <URL> , check it out !"
    )
...
def test_tokenize_2(tokenizer):
    ...
    assert (
        ...
        == "<RT> <USER> : Colorado's High Park Fire grows to <NUM> acres as hundreds flee - <USER> <URL>"
    )
...
def test_tokenize_3(tokenizer):
    ...
    assert (
        ...
        == "Video : <NUM> / <NUM> Containing Colorado's wildfire ; revisiting Watergate <URL> <HASH>"
    )
